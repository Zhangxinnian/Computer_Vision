...
def inception_padding_model(images, labels, wemb_size, seq_len, num_classes, lstm_size, is_training=True,
    ...
    with tf.compat.v1.variable_scope(name, reuse=reuse) as scope:
        regularizer = slim.l2_regularizer(weight_decay)  #l2正则化，权重衰减系数we....
        with slim.arg_scope(inception_v4_arg_scope()):
            with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
                ...
                cnn_feature = net    #inception V4网络对mask提取特征后的特征图

                output_array = tf.TensorArray(dtype=tf.float32, size=seq_len)  #Tensor数组
                batch, height, width, channel = cnn_feature.get_shape().as_list()  #从特征图中取出
                ...
                with tf.compat.v1.variable_scope("attention_lstm"):
                    with tf.compat.v1.variable_scope('word_embedding'):
                        W_wemb = tf.compat.v1.get_variable('W_wemb', [num_classes, wemb_size], initializer=xavier_initializer())#初始化词嵌入

                    with tf.compat.v1.variable_scope("feature_map_attention"):
                        with tf.compat.v1.variable_scope("init_mean"):   #转成向量输入到LSTM
                ...
                def _LSTMCell(wemb_prev, h_prev, attention_feature, c_prev, forget_bias=1., keep_prob=1.):
                    ...
                    pack_with_bias = tf.nn.bias_add(pack, lstm_b)   # (bsize, hid_dim * 4)#4倍的LSTM SZIE 的向量加入到矩阵PACK中，得到的大小和pack一致
                    i, f, o, g = tf.split(pack_with_bias, num_or_size_splits=4, axis=1) # (bsize, hid_dim)#把一个张量切分成四份
                    i = tf.sigmoid(i)
