...
def track(opt):
    ...
    while True:
        success, img = dataloader.cap.read()
        width = img.shape[1]
        height = img.shape[0]
        vw = opt.img_size[0]
        vh = opt.img_size[1]
        vw, vh = float(vw) / width, float(vh) / height
        a = min(vw, vh)
        img = cv2.resize(img, (int(width *a),int(height*a)))
        searchBboxes = cv2.selectROI("tracker", img)
        print(searchBboxes)
        k = cv2.waitKey(0)
        # print(k)
        if k == 32:
    ...
    try:
        eval_seq(searchBboxes,opt, dataloader, 'mot', result_filename,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='weights/jde.uncertainty.pt', help='path to weights file')
    parser.add_argument('--img-size', type=int, default=(640, 480), help='size of each image dimension')
