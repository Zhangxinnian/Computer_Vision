...
if __name__ == '__main__':
    ...
    with open('categoryv1.txt', 'w') as f:
        f.write('\n'.join(categories))
    ...
    for (filename_input, filename_output) in zip(files_input, files_output):
        ...
        for i in range(len(folders)):
            ...
            dir_files = os.listdir(os.path.join('/home/cj1/somethingv1/20bn-something-something-v1',curFolder))
            #output.append('%s %d %d' % ('something/v1/img/' + curFolder, len(dir_files), curIDX))
            output.append('%s %d %d' % ('/home/cj1/somethingv1/20bn-something-something-v1/' +curFolder,len(dir_files),curIDX))
